...
public class CallServer {
     ...
     * @param context   context用来实例化dialog
     * @param what      用来标志请求, 当多个请求使用同一个{@link HttpListener}时, 在回调方法中会返回这个what
     * @param myRequest   该项目二次封装的请求对象
     * @param callback  结果回调对象
     * @param canCancel 是否允许用户取消请求
     * @param isLoading 是否显示dialog
     */
    public <T> void add(Context context, int what, MyRequest<T> myRequest, HttpListener<NoResponse<T>> callback, boolean canCancel, boolean isLoading) {
        myRequest.packParams();
        requestQueue.add(what, myRequest, new MyResponseListener<T>(context, myRequest, callback, canCancel, isLoading));
    }
