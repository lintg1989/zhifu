...
//        playBeepSoundAndVibrate();
        ...
        if (result == null || TextUtils.isEmpty(result.getText())) {
            ...
            if (analyzeCallback != null) {
                boolean isRight = analyzeCallback.onAnalyzeSuccess(barcode, result.getText());
                if (isRight) {
                    playBeepSoundAndVibrate();
                    isDecode = true;
                }
    ...
    // 如果直接调用该方法，initCamera()中handler不为空，不会new CaptureActivityHandler
    // 在onPause时handler被置空（需要避免new CaptureActivityHandler）
